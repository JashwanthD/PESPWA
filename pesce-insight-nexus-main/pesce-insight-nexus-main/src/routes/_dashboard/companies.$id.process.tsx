...
function CompanyHiringProcess() {
  ...
  if (loading) return <div className="p-20 text-center animate-pulse text-zinc-500 uppercase tracking-widest font-black">Decrypting Recruitment Pathway...</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 space-y-8">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-black text-white uppercase tracking-tight">Recruitment Pathway</h1>
        <p className="text-[10px] text-zinc-500 uppercase tracking-[0.4em] font-black">Process Sequence / 2025-26 Cycle</p>
      </div>

      <div className="relative pl-12 space-y-12 before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-zinc-800 before:z-0">
        {rounds.map((round, idx) => (
          ...
          >
            {/* Timeline Node */}
            <div className="absolute -left-[53px] top-0 h-10 w-10 rounded-full bg-zinc-950 border-4 border-zinc-800 flex items-center justify-center text-white font-black text-xs shadow-xl">
              {idx + 1}
            </div>

            {/* Card */}
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-8 space-y-6 hover:border-indigo-500/30 transition-all group">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-2 text-zinc-500 text-[10px] font-black uppercase tracking-widest">
                  <Clock className="h-3.5 w-3.5 text-indigo-500" /> {round.duration}
                </div>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-8">
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Skills Evaluated</span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {round.skills.map(skill => (
                      <span key={skill} className="px-2 py-1 bg-indigo-500/5 border border-indigo-500/10 rounded text-[9px] font-bold text-zinc-400 uppercase tracking-widest">
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Intelligence Brief</span>
                  </div>
              ...
              <div className="pt-4 border-t border-zinc-800/50 flex items-center gap-2">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-zinc-600">Standard Pattern Verified</span>
              </div>
            </div>
          </motion.div>
        ))}
